...
const mutations = {
  ...
  [types.GET_CRYPTO_FULL_DATA_SUCCESS] (
    ...
    // process priceData
    const RAW = Object.values(priceData.RAW)[0] as any
    ...
    Vue.set(state, 'cryptoInfo', {
      current: {
        fiat: priceFiat,
        fiatChange: changeFiat,
        crypto: priceCrypto,
        cryptoChange: changePctCrypto
      },
      market: {
        cap: {
          fiat: marketcapFiat,
  ...
  [types.GET_ASSETS_RATES_REQUEST] (state) {},
  [types.GET_ASSETS_RATES_SUCCESS] (state) {},
  [types.GET_ASSETS_RATES_FAILURE] (state) {}
}
...
const actions = {
  ...
  async getAssetRates ({ commit, getters }, { assets, base }) {
    commit(types.GET_ASSETS_RATES_REQUEST)
    const dataCollectorUrl = getters.servicesIPs['data-collector-service']
    try {
      const res = await collectorUtil.getAssetRates(
        dataCollectorUrl.value,
        { assets, base, precision: 8 }
      )
      commit(types.GET_ASSETS_RATES_SUCCESS)
      return res
    } catch (error) {
      commit(types.GET_ASSETS_RATES_FAILURE)
      throw error
    }
  }
}
